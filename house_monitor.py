...
COMMUNITY = "天福元润国际"
DEFAULT_SOURCE_URL = (
    "https://fz.anjuke.com/sale/?comm_id=1193408&"
    "q=%E5%A4%A9%E7%A6%8F%E5%85%83%E6%B6%A6%E5%9B%BD%E9%99%85"
)
...
class MonitorApp:
    def __init__(self, root: Tk) -> None:
        ...
        # The supplied community-specific Anjuke result page is prefilled so that
        # users do not need to find or paste a search URL before their first check.
        self.url = StringVar(value=DEFAULT_SOURCE_URL)
        ...
        ttk.Button(form, text="打开网页", command=self.open_source).pack(side=RIGHT, padx=(8, 0))
    ...
    def open_source(self) -> None:
        """Open the configured source in the user's normal browser for verification."""
        url = self.url.get().strip()
        if url.startswith(("http://", "https://")):
            webbrowser.open_new_tab(url)
        else:
            messagebox.showwarning("需要 URL", "请输入以 http:// 或 https:// 开头的网页链接。")
